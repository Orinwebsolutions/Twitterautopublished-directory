...
if(!function_exists('xyz_twap_links')){
	function xyz_twap_links($links, $file) {
		...
		if ($file == $base) {
			...
			$links[] = '<a href="http://kb.xyzscripts.com/wordpress-plugins/twitter-auto-publish/"  title="FAQ">FAQ</a>';
			$links[] = '<a href="http://docs.xyzscripts.com/wordpress-plugins/twitter-auto-publish/"  title="Read Me">README</a>';
...
{
        $args = array('post_type'=> 'attachment','post_mime_type'=> 'image/jpeg','post_parent'=> $post_ID);
        $posts_array = get_posts( $args );//retrieving the image uploaded in the Gallery section
	$attachmenturl="";
	...
	if($post_thumbnail_id!="")
	{
		...
		$attachmentimage=wp_get_attachment_image_src( $post_thumbnail_id, full );
            ...
            $attachmenturl= $posts_array[0]->guid;//assigning the 1st image uploaded in the gallery
        }
	else {
